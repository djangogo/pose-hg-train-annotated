...
function getPreds(hm)
    ...
    -- 在第三个维度求max,也就是每一个heatmap上求max
    -- max的维度是hm:size(1),hm:size(2),1
    -- idx的维度同上
    local max, idx = torch.max(hm:view(hm:size(1), hm:size(2), hm:size(3) * hm:size(4)), 3)
    -- 所以pred是存放坐标的，所以最后一维是2
...
function distAccuracy(dists, thr)
    ...
    -- dists中不等于-1的所有元素的个数大于0则
    -- 实际上就是有距离的个数大于0
    if torch.ne(dists,-1):sum() > 0 then
        -- dists的元素中小于等于thr，并且不等于-1的个数占不等于-1的个数的比例
        -- 距离小于thr并且距离是有效的所占的比例
        -- dists:ne(-1):sum()是有效距离的元素的个数
        -- dists:le(thr):eq(dists:ne(-1)):sum()是小于阈值并且是有效距离的元素的个数
